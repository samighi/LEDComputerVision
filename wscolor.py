...
def wsshowLines(datafilename):
    ...
    for times in range(10):
        ...
        for ordered in ([x for x in sorted(l1,reverse=False)],[x for x in sorted(l1,reverse=True)],[x for x in sorted(l2,reverse=False)],[x for x in sorted(l2,reverse=True)]):
         ...
         #pixels.fill((0, 0, 0))
         for item in range(num_leds):
            strip.setPixelColor(item,0)
         p =(-1,-1,-1) 
         ...
         for i in rangevalues:
           ...
           for item in lit:
              ...
              #pixels[int(] = (0,0,0)
              strip.setPixelColor(v,Color(0,0,0))
           lit = []
           ...
           for j in range(len(ordered)):  
               if abs(ordered[j][0] - i) < vmax: 
                   ...
                   strip.setPixelColor(v,Color(255,255,255))
        ...
        # pixels.fill((0, 0, 0))
         ...
         for i in rangevalues:
           ...
           for item in lit:
              ...
              strip.setPixelColor(int(ordered[item][2]),Color(0,0,0))
           lit = []
           ...
           for j in range(len(ordered)):  
               ...
               if abs(distance) < vmax: 
                   ...
                   strip.setPixelColor(v,Color(128,128,128))

           strip.show()
           #time.sleep(.1)
           
def wsoutwardCircle(datafilename): 
    ...
    for ordered in [l1]:
     for times in range(5):
        ...
        # pixels.fill((0, 0, 0))
         ...
         print(ordered)
         xmin,xmax = min([x[0] for x in ordered]),max([x[0] for x in ordered])
         ymin,ymax = min([x[1] for x in ordered]),max([x[1] for x in ordered])
         XSEGMENTS ,YSEGMENTS = 16,8
         xinc = (xmax-xmin)/XSEGMENTS
         yinc = (ymax-ymin)/YSEGMENTS
         inc = xinc if xinc < yinc else yinc
         #erase the strip
         for item in range(num_leds):
            strip.setPixelColor(item,0)
         x1 = int((xmin+xmax)/2)
         ...
         for i in range(20):
          for item in range(num_leds):
            strip.setPixelColor(item,0)
            for j in range(len(ordered)):  
                x2 = ordered[j][0] ## X POINT of the pixel
                y2 = ordered[j][1] ## Y POINT of the pixel
              
                maxallowedDistance = (inc * (i+1))
                minallowedDistance = (inc * i )

                XFACTOR=20
                distance = ((x1-x2)**2+(y1-y2)**2)**0.5
                #if distance < maxallowedDistance and distance > minallowedDistance:  
                if distance <= maxallowedDistance:
                  #if distance >= minallowedDistance:
                     strip.setPixelColor(int(ordered[j][2]),Color(64,64,64) )
          strip.show()
          time.sleep(.2)
...
if __name__ == '__main__':
    num_leds = 150
    num_strand_leds = [100,100,100] # three strnds 
    datafilename = "orderednew.csv"
